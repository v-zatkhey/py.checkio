...
def sum_light(els: List[datetime], start_watching: Optional[datetime] = None) -> int:
    ...
    """
    if start_watching == None or start_watching < els[0]:
        return _sum_light(els)
    before_els = list(filter(lambda x: x < start_watching, els))
    rest_els = []
    if len(before_els) % 2 == 0:
        rest_els = els[len(before_els):]
    else:
        rest_els.append(start_watching)
        rest_els = rest_els + els[len(before_els):]
